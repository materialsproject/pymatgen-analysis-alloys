...
def ev_to_rgb(ev: float) -> tuple[int, int, int]:
...
def get_unfilled_valence_orbitals(ele: str | Element) -> list[tuple[int, str, int]]:
    ...
    for n in range(1, 10):
        for ell, symb in enumerate(orbital_symbols[:n]):
            subshells[n + ell].append((n, symb))
    ...
    unfilled_shells: list[tuple[int, str, int]] = []
    for subshell in filled_subshells:

        full_occ = [occupancy[orb[1]] for orb in subshell]
        occs = [occ_shells.get(orb[0], {}).get(orb[1], 0) for orb in subshell]
        ...
        if sum(occs) < sum(full_occ):
            ...
            if all(occ == full_occ[i] for i, occ in enumerate(occs) if occ > 0):
                ...
                unfilled_shells += [(*orb, occs[i]) for i, orb in enumerate(subshell) if occs[i] > 0]
            else:
                # Case 2: unfilled outermost shell, but at least one
                # orbital is unfilled.
                # Return only the unfilled orbitals within that subshell
                unfilled_shells += [(*orb, occs[i]) for i, orb in enumerate(subshell) if 0 < occs[i] < full_occ[i]]
...
def check_isoelectronicity(ele1: str | Element, ele2: str | Element) -> bool:
    ...
    """
    configs = [{orb[1]: orb[2] for orb in get_unfilled_valence_orbitals(ele)} for ele in (ele1, ele2)]
...
class AlloyPair(MSONable):
    ...
    Attributes:
        ...
        alloy_oxidation_state (Optional[float]): If set, will be the common oxidation state for
    ...
    alloying_species_a: str | None
    alloying_species_b: str | None
    observer_elements: list[str]
    observer_species: list[str] | None
    anions_a: list[str]
    anions_b: list[str]
    cations_a: list[str]
    cations_b: list[str]
    ...
    alloy_oxidation_state: float | None = field(repr=False)
    isoelectronic: bool | None
    ...
    def from_structures(
        ...
        ids: tuple[str, str],
        properties: tuple[dict[SupportedProperties, Any], dict[SupportedProperties, Any]] | None = None,
        ltol: float = LTOL,
        stol: float = STOL,
        ...
        formulas_and_structures = [
            ...
            for id_, s, s_oxi, property in zip(
                ids, structures, structures_with_oxidation_states, properties, strict=False
            )
        ]
    ...
    def _get_oxi_state_info(
        ...
        if (anions_a or cations_a) and (anions_b or cations_b):
            ...
            ions_a = [Species.from_str(sp) for sp in anions_a] + [Species.from_str(sp) for sp in cations_a]
            elements_a = [str(sp.element) for sp in ions_a]

            ions_b = [Species.from_str(sp) for sp in anions_b] + [Species.from_str(sp) for sp in cations_b]
    ...
    def get_property_with_vegards_law(self, x: float, prop: str = "band_gap") -> float | None:
        ...
        """
        prop_a = getattr(self, f"{prop}_a", None) or self.properties_a.get(prop)  # type: ignore[call-overload]
        prop_b = getattr(self, f"{prop}_b", None) or self.properties_b.get(prop)  # type: ignore[call-overload]
    ...
    def as_records(self, fields: list[str] | None = None) -> list[dict]:
    ...
    def search_dict(self) -> dict:
        ...
        # only works because we use this naming convention
        for attr in self.__dir__():
            if not attr.startswith("_") and (attr.endswith("_a") or attr.endswith("_b")):
                if isinstance(sample_attr := getattr(self, attr), float | int):
        ...
        for prop, value_a in self.properties_a.items():
            if isinstance(value_a, float | int):
        ...
        for prop, value_b in self.properties_b.items():
            if isinstance(value_b, float | int):
                numerical_properties.add(prop)
        ...
        for prop in numerical_properties:
            ...
            if prop_range:
                search[prop] = {"min": min(prop_range), "max": max(prop_range)}  # type: ignore[type-var]
...
class AlloySystem(MSONable):
    ...
    def systems_from_filter(
        self, pair_filter: Callable[[AlloyPair], bool], origin: str | None = None
    ) -> list[AlloySystem] | AlloySystem:
    ...
    def get_hull_trace_and_area(
    ...
    ) -> tuple[go.Trace | None, float | None]:
        ...
        if y_prop == "band_gap" and colour_by_centroid and (centroid is not None):
            try:
    ...
    def plot(
        ...
        plotly_pxline_kwargs: dict | None = None,
        plotly_pxscatter_kwargs: dict | None = None,
        plot_members: bool = True,
        member_plotly_pxscatter_kwargs: dict | None = None,
    ) -> go.Figure:
        ...
        for pair in self.alloy_pairs:
            data.append(
                {
                    column_mapping[x_prop]: getattr(pair, f"{x_prop}_a", None) or pair.properties_a.get(x_prop),  # type: ignore[call-overload]
                    column_mapping[y_prop]: getattr(pair, f"{y_prop}_a", None) or pair.properties_a.get(y_prop),  # type: ignore[call-overload]
                    column_mapping[symbol]: getattr(pair, f"{symbol}_a", None) or pair.properties_a.get(symbol),  # type: ignore[call-overload]
                    "id": pair.id_a,
            ...
            data.append(
                {
                    column_mapping[x_prop]: getattr(pair, f"{x_prop}_b", None) or pair.properties_b.get(x_prop),  # type: ignore[call-overload]
                    column_mapping[y_prop]: getattr(pair, f"{y_prop}_b", None) or pair.properties_b.get(y_prop),  # type: ignore[call-overload]
                    column_mapping[symbol]: getattr(pair, f"{symbol}_b", None) or pair.properties_b.get(symbol),  # type: ignore[call-overload]
                    "id": pair.id_b,
        ...
        if y_prop == "band_gap":
            for ev in np.arange(1.6, 3.1, 0.05):
                fillcolor = "rgb({},{},{})".format(*ev_to_rgb((ev + ev + 0.1) / 2))  # type: ignore[arg-type]
                fig.add_hrect(
                    y0=ev,
                    y1=ev + 0.05,
                    fillcolor=fillcolor,
...
class FormulaAlloyPair(MSONable):
    ...
    def from_pairs(cls, pairs: list[AlloyPair]) -> list[FormulaAlloyPair]:
        ...
        for _pair_formula, alloy_pair_group in alloy_pair_groups:
    ...
    def _get_hull(alloy_pair_df):
        ...
        for idx, (x, _y) in enumerate(hs.intersections):
            if not np.isnan(x):
    ...
    def plot(
        ...
        if not color_map:
            colors = (
                ...
                else px.colors.sample_colorscale(color_scale, n_colors, low=0, high=1)
            )
            color_keys = df[color].unique()
            color_map = {k: colors[idx] for idx, k in enumerate(color_keys)}
            if len(color_keys) > len(colors):
                raise ValueError(
                    "The number of alloy pairs exceeds " "the number of colors. Please supply your own color map!"
                )
            ...
            if mp_member_ids:
                with MPRester(api_key) as mpr:
                    e_above_hulls = {
                        ...
                        for doc in mpr.thermo.search_thermo_docs(material_ids=list(mp_member_ids))
                    }

                for pair in self.pairs:
                    for _i, member in enumerate(pair.members):
                        if member.db == "mp":
        ...
        y_lim = y_limit if y_limit is not None else df["energy_above_hull"].max()
        fig.update_yaxes(range=(-y_lim * 0.15, y_lim + (y_lim * 0.15)))
        fig.update_layout(
            autosize=False,
    ...
    def _get_decomp_df(
        ...
        """Create a dataframe of thermodynamic decomposition products for a given FormulaAlloyPair
    ...
    def _add_halfspace_hull_shading(
        ...
        """Add a shaded halfspace hull window to FormulaAlloyPair plot.
        ...
        for n in range(shades - 1):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    fill="tonexty",
                    fillcolor=f"rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, {opac})",
                    line=dict(
                        color=f"rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, {opac})",
                        width=0,
                    ),
                    showlegend=False,
                    name="hull_shading",
    ...
    def get_critical_points_data(fig):
        """Get the critical points of the halfspace hull for a FormulaAlloyPair figure.
...
def _get_colormap_from_keys(color_keys, color_scale="greys", range_of_scale=(0.1, 0.9)):
    """Write a plotly colormap from a set of keys.
    ...
    colors = px.colors.sample_colorscale(color_scale, n_colors, low=range_of_scale[0], high=range_of_scale[1])
    color_map = {k: colors[idx] for idx, k in enumerate(color_keys)}
